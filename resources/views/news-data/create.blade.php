    ...
    <div class="col-lg-12">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                Add News <a href="{{ route('news.index') }}" class="label label-primary pull-right">Back</a>
            </div>
            <div class="panel-body">
                <form action="{{ route('news.insert') }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label class="control-label col-sm-2" >News Title</label>
                        <div class="col-sm-10">
                            <input type="text" required name="news_title" id="news_title" class="form-control" value="{{ old('news_title') }}">
                        </div>
                    </div>
                            ...
                            <select name="news_category_id" id="news_category_id" class="form-control" required>
                                ...
                                @if(!empty($categories))
                                    @foreach($categories as $key=>$val)
                                        <option @if($key == old('news_category_id')) selected @endif value="{{ $key }}">{{ $val }}</option>
                                    @endforeach
                                @endif
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <textarea name="news_description" rows="10" required id="news_description" class="form-control">{{ old('news_description') }}</textarea>
                        </div>
                    </div>
